...
function getOperationsPerId($id){
    if(isContributor()) return (DB::select('select operation.id, operation.message, operation.state, operation.timestamp, operation.note, operation.prix from contributor join users on users.id = userid join operation on operation.contributorid = contributor.id where contributor.id = ' . $id));
